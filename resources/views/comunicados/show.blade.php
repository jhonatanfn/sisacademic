<x-app-layout>
    <x-nav-comunicado :categorias="$categorias"/>
    
    <div class="container py-8">
        <h1 class="text-4xl font-bold text-gray-600">{{$comunicado->titulo}}</h1>

        <div class="text-lg text-gray-500 mb-2">
            {!! $comunicado->extracto !!}
        </div>
       
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
             {{-- contenido principal --}}
            <div class="lg:col-span-2">
                
                <figure>
                   @if ($comunicado->image)
                    <img class="w-full h-80 object-cover object-center" 
                    src="{{Storage::url($comunicado->image->url)}}" alt="">
                   @else
                    <img class="w-full h-80 object-cover object-center" 
                    src="{{Storage::url('default/comunicado.jpg')}}" alt="">
                   @endif
                </figure>

                <div class="text-base text-gray-500 mt-4">
                    {!! $comunicado->cuerpo !!}
                </div>
  
            @livewire('comment-livewire',['comunicado_id'=>$comunicado->id,'band'=>1])

            </div>
            {{-- contenido relacionado --}}
            <aside>
                <h1 class="text-2xl font-bold text-gray-600 mb-4">
                    Más en {{$comunicado->categoria->nombre}}</h1>
                <ul>
                    @foreach ($similares as $similar)
                        <li class="mb-4">
                            <a class="flex" href="{{route('comunicados.show',$similar)}}">
                                @if ($similar->image)
                                    <img class="w-36 h-20 object-cover object-center" 
                                    src="{{Storage::url($similar->image->url)}}" alt="">
                                @else
                                    <img class="w-36 h-20 object-cover object-center" 
                                    src="{{Storage::url('default/comunicado.jpg')}}" alt="">
                                @endif
                                <span class="ml-2 text-gray-600">{{$similar->titulo}}</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
            </aside>
        </div>

    </div>
</x-app-layout>